...
async function createForm(formURL) {
  ...
  form.addEventListener('submit', (event) => formsubmissionHandler(form, event));
  return form;
}
...
function validateFormElements(form) {
  let validate = true;
  [...form.elements].forEach((fe) => {
    const isRequired = fe.getAttribute('required') === 'required';
    if (isRequired) {
      if (fe.type === 'fieldset') {
        const inputElements = fe.querySelectorAll('input');
        let isEmp = true;
